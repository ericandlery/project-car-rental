...
<!--                       HTML改這裡 ////////////////////-->
		...
		<div id="midden">
			<table id="bigtable" border=1 RULES=ROWS>
				<tr id="bigtable_head" >
					<td colspan="5">搜尋姓名</td>
				</tr>
				<tr>
					<form action="searchname.controller" method="post">
						<td style="colspan:3;width:320px;">
							請輸入姓名搜尋:<input type="text" name="name" value="${name}">
		        		</td>
		        		<td><input id="goodbutton" type="submit" value="搜尋"></td>
	        			<td colspan="3"></td>
	        		</form>
	        	</tr>
	        	<c:choose>
        				<c:when test="${listnameNULL=='查無資料'}">
					        	<tr><td colspan="5">${listnameNULL}</td>
					    </c:when>
        		</c:choose>
				<c:forEach var="row" items="${listname}">
			<form action="addinvitetoaddnewfriend.controller" method="post">
			<tr>
			<td><img style="margin-left:30px;width:100px;height:100px;" src="/TestRentalMVC/member_photo/${row[0]}"></td>	
<%-- 			<td style="width:80px;padding:5px;"><img style="width:80px;" src="/TestRentalMVC/images/${row[0]}"></td> --%>
			<input type="hidden" name="id" value="${row[1]}">
<%-- 			<td>${row[1]}</td> --%>
			<td style="width:80px;padding:0;">${row[2]}</td>
			<td style="width:80px;padding:0;">${row[3]}</td>
<%-- 			<td style="width:80px;border:2px solid;padding:0;">${row[4]}</td> --%>
			<input type="hidden" name="relationship" value="${row[4]}">
			<c:choose>
			  ...
			  <c:when test="${row[4]=='邀請者'}">
			  	<td><input type="submit" value="接受邀請"></td>
			  </c:when>
			  <c:otherwise>
