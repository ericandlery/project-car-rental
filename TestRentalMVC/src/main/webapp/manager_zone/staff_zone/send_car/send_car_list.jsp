    ...
    <script>
    	$(document).ready(function(){
    		$("#futureSubmit").click(function(event){
    			...
    			var b=confirm("這不是今天的訂單，確定要現在處理嗎？");
    			if(b==true){
    				$("#futureForm").submit();
    			}else if(b==false){
...
<!--                       改這裡 -->
		<div id="manager">
                <div id="title"><a href="/TestRentalMVC/manager_zone/staff_zone/staff_zone.jsp">主選單</a>>>${staff_login_ok.branch_name}門市取車頁面</div>
                ...
                <c:choose>
                	...
                	<c:when test="${not empty futureList}">
                		<table>
                			<tr><th>訂單編號</th><th>取車據點</th><th>還車據點</th><th>取車時間</th><th>訂單狀態</th></tr>
                			<c:forEach var="future" items="${futureList}">
                				<tr><td>${future.id}</td><td>${future.start_branch_name}</td><td>${future.end_branch_name}</td>
                					<td>${future.start_date}</td><td>${future.order_state}</td>
<!--                 					<td> -->
<!--                 					<form id="futureForm" action="dealOrder.controller" method="post"> -->
<%--                 					<input type="hidden" value="${future.id}" name="id"> --%>
<!--                 					<input  id="futureSubmit" type="submit" value="完成取車"> -->
<!--                 					</form> -->
<!--                 					</td> -->
                					</tr>
                			</c:forEach>
                		...
                		<table>
                			...
                			<c:forEach var="today" items="${todayList}">
                				<tr><td>${today.id}</td><td>${today.start_branch_name}</td><td>${today.end_branch_name}</td>
                					<td>${today.start_date}</td><td>${today.order_state}</td>
                					<td>
                					<form action="dealOrder.controller" method="post">
                					<input type="hidden" value="${today.id}" name="id">
                					<input id="todaySubmit" type="submit" value="完成取車" onclick="confirmOrder(event)">
                					</form>
                					</td></tr>
                			</c:forEach>
                		</table>
                ...
                <div class="box"><span class="boxTitle">過期的訂單</span>
				<c:choose>
                		...
                		<table>
                			...
                			<c:forEach var="old" items="${oldList}">
                				<tr><td>${old.id}</td><td>${old.start_branch_name}</td><td>${old.end_branch_name}</td>
                					<td>${old.start_date}</td><td>${old.order_state}</td>
                					<td>
                					<form action="cancelOldOrder.controller" method="post">
                					<input type="hidden" value="${old.id}" name="id">
                					<input id="oldSubmit" type="submit" value="取消訂單">
                					</form>
                					</td></tr>
                			</c:forEach>
                		</table>
